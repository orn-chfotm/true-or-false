...
이 문서는 Java DTO의 형태, Entity 변환과 공통 응답 래퍼를 정의한다.
...
- Swagger 문서화 규칙 (springdoc-openapi) 참조: @.claude/rules/backend/swagger/01-api-docs.md
...
- QueryDSL 규칙 참조: @.claude/rules/backend/jpa/04-querydsl.md
...
- Entity에 `@Setter`를 두지 않는다. 상태는 도메인 메서드로만 변경한다.
