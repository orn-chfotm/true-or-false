...
Opinion Brief의 Service 컨벤션이다. Service는 `domain` 모듈의 도메인별 `service`에 둔다(`../00-module-structure.md`). 트랜잭션 경계는 Service 계층에서 명확히 한다(하네스 `.ai-prompts/docs/10-technical/02-spring.md`).
...
- ❌ 필드 주입을 쓰지 않는다. `final` + `@RequiredArgsConstructor` 생성자 주입.
