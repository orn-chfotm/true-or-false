...
이 문서는 enum과 값 타입의 DB 저장 매핑 및 AttributeConverter 사용 기준을 정의한다.
...
- 컨버터에서 null 처리를 누락하지 않는다. 알 수 없는 코드는 예외로 처리한다.
- `@Converter(autoApply = true)`를 무분별하게 전역 적용하지 않는다. 특정 필드에만 적용하면 `@Convert`를 명시한다.
