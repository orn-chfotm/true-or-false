...
이 스킬은 FSD의 레이어·세그먼트·공개 API 기준에 맞게 slice를 배치하는 절차다.
...
- 하위 레이어가 상위 레이어를 참조하거나 같은 레이어의 다른 slice를 직접 참조하지 않는다. 의존 방향은 위에서 아래로만 허용한다.
- 외부에 slice 내부를 deep import로 노출하지 않는다. `index.ts`로만 노출한다.
