...
Opinion Brief 백엔드의 도메인 모델(엔티티, 상태 전이, 연관)이 `.claude/rules/01~06`을 지키는지 점검하는 절차다.
...
1. 대상 엔티티와 관련 룰 파일을 매핑한다.
   - 도메인/상태/신뢰점수 → `01-domain-model.md`
   - 검수/AI 로깅 → `02-review-pipeline.md`
   - 리포트 → `03-report-snapshot.md`
   - 보상 → `04-reward-ledger.md`
   - 슬롯/동시성 → `05-participation-slot.md`
   - 개인정보 → `06-privacy.md`
2. 아래 체크리스트로 위반을 찾는다.
